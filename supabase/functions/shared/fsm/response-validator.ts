...
const EXTRA_PATTERNS: RegExp[] = [
  ...
  // RU — 6 pattern
  // 2026-07-09 FABLE-denetim: \b Kiril'de sınır TANIMAZ → 6 RU sahte-ack
  // pattern'i HİÇ fire etmiyordu (V1-ack guard'ı RU'da ölüydü, Yan #8) → lookaround.
  /(?<![\p{L}\p{N}])(ваше?\s+)?бронирование?\s+(подтверждено|сохранено|создано|завершено|оформлено)/iu,
  /(?<![\p{L}\p{N}])мы\s+(получили|подтвердили|обработали)\s+ваш[еу]?\s*(бронирование|заявку|запрос)/iu,
  /(?<![\p{L}\p{N}])(успешно)\s+(забронировано|подтверждено|обработано|оформлено)/iu,
  /(?<![\p{L}\p{N}])ваш[аея]?\s+(поездка|тур)\s+(подтвержден[оа]?|забронирован[оа]?)/iu,
  /(?<![\p{L}\p{N}])наш[аи]?\s+сотрудники?\s+(свяжутся|позвонят|напишут)/iu,
  /(?<![\p{L}\p{N}])поздравляем.{0,40}(бронирование|тур|поездка)/iu,

  // AR — 6 pattern
  // 2026-07-09 FABLE-denetim: aynı Yan #8 — 6 AR pattern ölüydü → lookaround.
  /(?<![\p{L}\p{N}])(تم\s+)?(تأكيد|حجز|تسجيل)\s+(طلبك|حجزك|رحلتك)/iu,
  /(?<![\p{L}\p{N}])(تلقينا|أكدنا|عالجنا)\s+(حجزك|طلبك)/iu,
  /(?<![\p{L}\p{N}])(بنجاح)\s+(محجوز|مؤكد|معالج|مسجل)/iu,
  /(?<![\p{L}\p{N}])حجزك\s+(مؤكد|مكتمل|تم|أنجز)/iu,
  /(?<![\p{L}\p{N}])فريقنا\s+سيتواصل\s+(معك|بك)/iu,
  /(?<![\p{L}\p{N}])تهانينا.{0,40}(الحجز|الرحلة|الجولة)/iu,
];
